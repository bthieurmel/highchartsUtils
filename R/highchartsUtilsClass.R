...
#' @import yaml
#' @import base64enc
#' @export highchartsUtils
highchartsUtils = setRefClass('highchartsUtils', list(params = 'list', lib = 'character', 
    LIB = 'list', srccode = 'ANY', tObj = 'list', container = 'character', 
    html_id = 'character', templates = 'list', html_assets = 'list', addFunctionJS = 'character', 
    htmlheader = 'character', myChartData = 'character', myRefChartData = 'character', myLevels = 'character', scope = 'ANY'),
  methods = list(
  ...
  initialize = function(){
    ...
    templates <<- list(page = 'highcharts.html', chartDiv = NULL, afterScript = "<script></script>",
      script =  file.path(LIB$url, 'layouts', 'chart.html'))
    templates$chartDiv <<- "<{{container}} id = '{{ chartId }}' class = 'highchartsUtils {{ lib }}'></{{ container}}>"
  },
  ...
  functionsJS = function(value){
    addFunctionJS <<- value
  },
  
  htmlHeader = function(value){
    htmlheader <<- value
  },
  
  levels = function(value){
    if(!identical(value, "")){
      myLevels <<- paste("var levels = new Array('", paste(value, collapse = "','"), "');", sep="" )
    }else{
      myLevels <<- ""
    }
  },
  ...
  chartData = function(value){
    if(!identical(value, "")){
      myChartData <<- paste("var data = ", gsub('\"',"'",value), ";", sep="")
    }else{
      myChartData <<- ""
    }
  },
  
  RefchartData = function(value){
    if(!identical(value, "")){
      myRefChartData <<- paste("var refdata = ", gsub('\"',"'",value), ";", sep="")
    }else{
      myRefChartData <<- ""
    }
  },
  ...
  addControls = function(nm, value, values, label = paste("Select ", nm, ":")){
    .self$setTemplate(
      page = 'highchartsControls.html',
      script = system.file('libraries', lib, 'controls', 
        'script.html', package = 'highchartsUtils')
    )
    .self$set(width = 700)
  ...
  setLib = function(lib, ...){
    ...
    templates <<- modifyList(list(
      page = 'highcharts.html', 
      chartDiv = "<{{container}} id = '{{ chartId }}' class = 'highchartsUtils {{ lib }}'></{{ container}}>", 
      script =  file.path(LIB$url, 'layouts', 'chart.html')
    ), list(...))
  },
  set = function(...){
  ...
  render = function(chartId = NULL, cdn = F, static = T, standalone = F){
    ...
    assets = Map("c", 
                 get_assets(LIB, static = static, cdn = cdn, standalone = standalone, scope = scope), 
                 html_assets
    )
  ...
  show = function(mode_ = NULL, ..., extra_files = NULL){
    ...
    switch(mode_, 
       static = {
         dir.create(temp_dir <- tempfile(pattern = 'highchartsUtils'))
      ...
      server = {
        ...
        assign(".highchartsUtils_object", shiny_copy, envir = .GlobalEnv)
        ...
        shiny::runApp(
          file.path(system.file(package = "highchartsUtils"), "shiny"),
      ...
      iframe = {
        ...
        cat(c(
          "<iframe src='", file_, 
          "' scrolling='no' frameBorder='0' seamless", paste("class='highchartsUtils", lib, "'"),
      ...
      iframesrc = {
        cat(c(
          ...
          "' scrolling='no' frameBorder='0' seamless class='highchartsUtils ", lib, " '",
...
getMode = function(mode_){
  ...
  mode_ = mode_ %||% getOption('highchartsUtils.mode') %||% default
  return(mode_)
}
