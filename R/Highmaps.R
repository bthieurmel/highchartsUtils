...
#' @export Highmaps
Highmaps <- setRefClass("Highmaps", contains = "highchartsUtils", methods = list(
    initialize = function()
    {
        callSuper(); lib <<- 'highmaps'; LIB <<- get_lib(lib)
    ...
    getPayload = function(chartId){
        ...
        list(chartData = myChartData, RefchartData = myRefChartData, levels = myLevels, chartParams = toJSON2(params, digits = 13), chartId = chartId)
    },
    ...
    # >>> Specific to Highmaps
  
    mapNavigation = function(..., replace = T) {
      params$mapNavigation <<- setSpec(params$mapNavigation, ..., replace = replace)
    },
