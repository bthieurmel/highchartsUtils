...
get_lib <- function(lib, package = 'highchartsUtils'){
  if (grepl("^http", lib)){
...
open_notebook <- function(rmdFile = NULL){
  ...
  app <- system.file('apps', 'notebook', package = 'highchartsUtils')
  shiny::runApp(app)
}
...
get_assets <- function(LIB, static = T, cdn = F, standalone = F, scope = NULL){
  config = yaml.load_file(file.path(LIB$url, 'config.yml'))[[1]]
  if(!standalone){
    if (getOption('highchartsUtils.cdn', cdn)) {
      result = config$cdn 
      if(!is.null(scope)){
        addscope <- config[[scope]]$cdn
        result[[1]] <- c(result[[1]], addscope[[1]])
      }
    } else {
      assets = config["jshead"]
      if(!is.null(scope)){
        addscope <- config[[scope]]$jshead
        assets[[1]] <- c(assets[[1]], addscope)
      }
      if(static){
        prefix = paste('file:///',LIB$url,sep="")
        result = lapply(assets, function(asset) {
          gsub("[[:space:]]+","%20",paste(prefix, asset, sep = '/'))
        })
      }else{
        prefix = LIB$name
        result = lapply(assets, function(asset) {
...
#' 
#' @param file path to text file that needs to be read
#' @param warn logical. Warn if a text file is missing a final EOL
#' @param ... other parameters to be passed to \code{\link{readLines}}
#' @keywords internal
#' @noRd
...
read_template <- function(..., package = 'highchartsUtils'){
  if (is.null(package)){
    template = file.path(...)
  } else {
